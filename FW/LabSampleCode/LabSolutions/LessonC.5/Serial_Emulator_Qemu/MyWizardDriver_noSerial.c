...
UpdateNVVariable()
{
	...
	if !(EFI_ERROR(Status)) {  // Did not Find so can not update
			...
			DEBUG((DEBUG_INFO, "***[MyWizardDriver] Variable %s Updated in NVRam Var\n", mVariableName));
			return EFI_SUCCESS;
	}
	// 
	return Status;
  ...
  )
{
	...
	if (EFI_ERROR(Status)) {
		...
		if (EFI_ERROR(Status)) {
			...
			DEBUG((DEBUG_INFO, "[MyWizardDriver] Supported \r\n"));
		}
  ...
  )
{
	...
	if (FirstAlloc) {
	  ...
	  StrCpyS(mMyWizDrv_Conf_buffer.MyWizardDriverStringData, (MYWIZARDDRIVER_STRING_SIZE * sizeof(CHAR16)), L"UEFI-Training-Class-MWD");

	  // Update the NVRAM Variable MWD_NVData
	  Status = UpdateNVVariable();
	  if (EFI_ERROR(Status)) {
		DEBUG((DEBUG_INFO, "***[MyWizardDriver] Error in updating the NV Variable  \n"));
 	  }
	  else {
