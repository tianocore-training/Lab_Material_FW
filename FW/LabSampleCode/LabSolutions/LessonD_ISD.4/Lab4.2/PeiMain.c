  ...
  )
{
  ...
  DEBUG((DEBUG_INFO, "start of PeiCore \n"));
  DEBUG((DEBUG_INFO, "\n>>>>>[PeiCore] Entry point: 0x%p <<<<<<\n", PeiCore));
  CpuDeadLoop();
  ...
  if (OldCoreData == NULL) {
	  //  EDK II Training ISD Lab
	  //  Add CpuDeadLoop() to the code
		  DEBUG((DEBUG_INFO, "\n>>>>>[PeiCore]start of PeiCore First Entry  before memory is available <<<<<< \n"));
		  CpuDeadLoop();
    //
    ...
    CopyMem (&PrivateData.ServiceTableShadow, &gPs, sizeof (gPs));
	  ...
	  //  EDK II Training ISD Lab
	  //  Add CpuDeadLoop() to the code
	  DEBUG((DEBUG_INFO, "\n>>>>>[PeiCore]start of PeiCore After First Entry <<<<<< 0x%p\n", OldCoreData->ShadowedPeiCore));
	  CpuDeadLoop();
	//
    ...
    if (OldCoreData->ShadowedPeiCore == NULL) {
		...
		//  EDK II Training ISD Lab
		//  Add CpuDeadLoop() to the code
		DEBUG((DEBUG_INFO, "\n>>>>>[PeiCore] PeiCore Second Entry  ShadowedPeiCore == NULL <<<<<< \n"));
		CpuDeadLoop();
	  ...
	  //  EDK II Training ISD Lab
	  //  Add CpuDeadLoop() to the code
	  DEBUG((DEBUG_INFO, "\n>>>>>[PeiCore] PeiCore before call to Restart PEI in Core Memory <<<<<< \n"));
	  CpuDeadLoop();
      //
  ...
  //  EDK II Training ISD Lab
  //  Add CpuDeadLoop() to the code
  DEBUG((DEBUG_INFO, "\n>>>>>[PeiCore]  Before Call to Dispatcher<<<<<<\n"));
  CpuDeadLoop();
