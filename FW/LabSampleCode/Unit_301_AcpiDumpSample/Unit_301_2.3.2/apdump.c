    ...
    void)
{
	...
	//printf("Check Acpi status==%x and loop counter i==%d \n", Status, i )  ;
        if (ACPI_FAILURE (Status))
        {
            /* AE_LIMIT means that no more tables are available */
            if (Status == AE_LIMIT) {
				if (i == 0 ){
                  	return (2);   // No ACPI Tables - Perhaps in NT32 Emulation
				} else {
					return (0);  // SUCESS 
				}
            }
            ...
            {
				printf ("IN ACPI Dump not supported - perhaps running NT32 Emulation\n Could not get ACPI tables\n");
