...
#include "ScsiDisk.h"
```
## Module Data Structures for Binding Protocol and SCSI Dick Info Protocol - Produced
```
...
**/
```
## Entry point, notice only Install of Binding and Component Name Protocol
```
EFI_STATUS
EFIAPI
InitializeScsiDisk(
...
**/
```
## The Supported Function, Only Checks and returns Supported or Unsupported
```
EFI_STATUS
EFIAPI
ScsiDiskDriverBindingSupported (
  ...
  )
{
  ...
  if (!EFI_ERROR (Status)) {
    ...
    //
```
## Start calls Install function to create handles for Multiple Protocols
```
...
```
## Stop Calls Un-install function matching the Start's Install
```
  if (!EFI_ERROR (Status)) {
